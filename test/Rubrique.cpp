...
SousRubrique :: SousRubrique(int id,string theme,string dateCreation, string adresseEmailAnimateur,int nb):Rubrique(id_rubrique,date_creation,ad_mail_anima,theme),nb_abonnee(nb){}
void SousRubrique ::AddMsg(Message m)
{
    TabMsg.push_back(m);
}
void SousRubrique ::AfficherMsgsSelonTitre(Mestitres t)
{
    for(int i=0;i<TabMsg.size();i++)
    {
        if(TabMsg[i].get_Titre()==t)
            cout<<TabMsg[i];
    }
}
